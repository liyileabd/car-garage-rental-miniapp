...
(() => {
  ...
  const ANNOTATIONS = [
    /* ---------------- 首页 ---------------- */
    {
      ...
      body: L(
        "点击此处跳转登录页。",
        "未登录状态下无法使用各项功能，点击任意入口均跳转至登录页。"
      )
    },
    /* 以下几条是「登录后」页面上的功能说明，都标 authed ——
       未登录时这些入口点了会被拦到登录页，讲它们是什么没有意义，
       所以未登录首页只留上面那条登录说明 */
    /* 月租卡有两种互斥状态（有效 / 暂未月租），分别锚在各自的状态块上，
       只有对应状态出现时圆点才在 —— 锚整张 .rent-card 的话，
       「暂未月租」时也会弹出讲生效车位的说明 */
    {
      id: "homeRentCard",
      screen: "home",
      auth: "authed",
      anchor: "[data-home-rent-active]",
      body: L(
        "当前正在生效的月租车位。",
        "右上角「**续费**」按钮：续费不改动当前订单，而是**另生成一条续费订单**（待生效）。",
        "新租期自当前租期结束次日起算，到期自动接上。"
      )
    },
    {
      id: "homeRentEmpty",
    ...
    /* ---------------- 办理月租 · 选择小区 ---------------- */
    {
      ...
      body: L(
        ...
        "总车位数在创建项目时由工作人员录入；可办数由系统按已租数量自动扣减，工作人员也可在 PC 端管理后台调整。"
      )
    },
    ...
    /* ---------------- 我的订单 ---------------- */
    {
      ...
      anchor: ".order-tabs",
      body: L(
        "按订单所处阶段筛选：进行中为租期有效或待生效的订单，售后为退款处理中的订单，已结束为到期、终止或退款完成的订单。",
        "进行中与已结束下另有细分标签，随所选分类出现。"
      )
    },
    {
      id: "orderCard",
      screen: "orderList",
      auth: "authed",
      anchor: "[data-order-list]",
      body: L(
        "每张卡片是一笔月租订单。",
        "底部操作项随订单当前状态提供，可续费、变更车辆、申请退款或开票。"
      )
    },
    {
      ...
      body: L(
        "开票申请从符合条件的订单发起，凭证类型由系统按所属项目与收费项目自动判定，无需用户选择票据种类。",
        "一张订单可对应多张发票或非税票据，全部票据在开票记录中查看。"
      )
    },
    ...
    /* ---------------- 申请退款 ---------------- */
    {
      id: "refundAccountEntry",
      screen: "orderRefund",
      auth: "authed",
      anchor: "[data-refund-account-open]",
      body: L(
        "退款账户为必填项，未填写时无法提交退款申请。",
        "曾填写过的，默认带出上次保存的信息，可修改；未填写过则为空。"
      )
    },
    {
      id: "refundListRule",
      ...
      anchor: ".refund-tabs",
      body: L(
        "退款申请由 **PC 端管理后台**审核并核定金额；小程序展示的金额由系统按剩余租期测算，最终以工作人员核定的为准。",
        "后台未调整金额的，直接给出审核结果；调整过金额的，退回小程序由用户确认。",
        "审核通过后为**线下退款**，小程序不再收到退款完成的回传。"
      )
    },
