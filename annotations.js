...
(() => {
  ...
  const ANNOTATIONS = [
    /* ---------------- 登录 ---------------- */
    {
      ...
      title: "微信快捷登录 = 有数据，验证码登录 = 没有数据",
      body: L(
        "两个入口进的是**不同的账号**，看到的东西完全不一样：",
        "",
        "· **微信快捷登录** → **有数据**的账号",
        "  订单、退款记录、车辆、开票记录都是完整的。",
        "  （会先跳到「关联手机号」页，填完才落地）",
        "",
        "· **验证码登录** → **没有数据**的账号",
        "  资料是空的，订单列表和退款记录也都是空的。"
      ),
      meta: {
        两份数据: "app.js:959 · accountState（orders / refunds 各存一份）",
        切换函数: "app.js · setCurrentAccount()"
      },
      tags: ["登录", "演示数据"]
    }
  ];
